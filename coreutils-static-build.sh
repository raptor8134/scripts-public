...
if [ "$platform" = "Linux" ]; then
  ...
  install_dir='/data/user/0/ru.iiec.pydroid3/files'

  pushd musl-${musl_version}
  env CFLAGS="$CFLAGS -Os -ffunction-sections -fdata-sections" LDFLAGS='-Wl,--gc-sections' ./configure --prefix=${install_dir} --host=i686
  #make install
